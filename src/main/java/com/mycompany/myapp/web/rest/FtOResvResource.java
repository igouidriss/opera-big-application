...
import com.mycompany.myapp.service.FtOResvService;
...
/**
 * REST controller for managing {@link com.mycompany.myapp.domain.FtOResv}.
 */
@RestController
@RequestMapping("/api/ft-o-resvs")
public class FtOResvResource {
    ...
    public ResponseEntity<FtOResvDTO> createFtOResv(@RequestBody FtOResvDTO ftOResvDTO) throws URISyntaxException {
        ...
        ftOResvDTO = ftOResvService.save(ftOResvDTO);
        return ResponseEntity.created(new URI("/api/ft-o-resvs/" + ftOResvDTO.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, ftOResvDTO.getId().toString()))
            .body(ftOResvDTO);
    }
    ...
    public ResponseEntity<FtOResvDTO> updateFtOResv(
        ...
        ftOResvDTO = ftOResvService.update(ftOResvDTO);
     ...
     */
    @GetMapping("/{id}")
    public ResponseEntity<FtOResvDTO> getFtOResv(@PathVariable("id") Long id) {
     ...
     */
    @DeleteMapping("/{id}")
    public ResponseEntity<Void> deleteFtOResv(@PathVariable("id") Long id) {
